    ...
    <div class="container mb-3 mt-3">
        <section>
            <div class="btn-group" role="group" aria-label="Basic example">
                <a href="{{route('admin.ListReservas')}}" type="button" class="btn btn-success">Voltar</a>
            </div>
        </section>
        <div>
            @foreach($var as $v)
            <h2 class="mt-3">Lista de livros reservados do(a) {{$v->UserReserva->name}}</h2>
            @endforeach
        </div>
        <table class="table mb-3 mt-3"  style="max-width: 1200px;margin: auto">
            ...
            @foreach($var as $v)
                <tbody>
                <tr>
                    <th scope="col">{{$v->LivroReserva->id}}</th>
                    <th scope="col">{{$v->LivroReserva->isbn}}</th>
                    <th scope="col">{{$v->LivroReserva->titulo}}</th>
                </tr>
                </tbody>
            @endforeach
        </table>
