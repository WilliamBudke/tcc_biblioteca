...
<h2 class="mt-3 mb-3 d-flex justify-content-center">Lista de Reserva de Livros</h2>
<main>
...
<table class="table mb-3 mt-3"  style="max-width: 1200px;margin: auto">
    ...
    <tbody>
        <tr>
            ...
            <td>{{$r->status}}</td>
            <td>{{$r->UserReserva->name}}</td>
            <td>
                <form action="{{route('admin.LocarReserva',['id'=> $r->id])}}" method="post">
                    @csrf
                    <button type="submit" class="btn btn-info">Locar reserva</button>
                </form>
            </td>
            <td>
                <form action="{{route('admin.LivrosReserva',['id'=> $r->id])}}" method="post">
                    @csrf
